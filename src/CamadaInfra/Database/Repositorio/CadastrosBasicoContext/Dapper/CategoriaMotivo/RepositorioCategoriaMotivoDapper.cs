...
namespace CamadaInfra.Database.Repositorio.CadastrosBasicoContext.Dapper.CategoriaMotivo
{
    public class RepositorioCategoriaMotivoDapper : RepositorioBaseDapper<CategoriaMotivoOutputModel, FiltroCategoriaMotivoInputModel>, IRepositorioCategoriaMotivoLeitura
    {
        public RepositorioCategoriaMotivoDapper(IConfiguration config) : base(config)
        {
        }
        

        public override async Task<Option<IEnumerable<CategoriaMotivoOutputModel>>> ListarAsync(FiltroCategoriaMotivoInputModel filtros)
        {
            List<CategoriaMotivoOutputModel> retorno = new List<CategoriaMotivoOutputModel>();
            //retorno.TotalRegitros = await RetornarInteiroAsync(BuscarQuantidadeCidades(filtros));            

            return await RetornarTodosAsync(BuscarCategoriaMotivo());
        }

        private string BuscarQuantidadeCategoriaMotivo(FiltroCategoriaMotivoInputModel filtros)
        {
            var sb = new StringBuilder();
            sb.AppendLine("SELECT Count(mot.Id) FROM CategoriasMotivo cat");
        ...
        private string BuscarCategoriaMotivo()
        {
            ...
            sb.AppendLine(" cat.Nome");
            sb.AppendLine(" from CategoriasMotivo cat");
            sb.AppendLine(RetornarJoins());
        ...
        private string Filtrar(FiltroCategoriaMotivoInputModel filtro, bool ordenar = true)
        {
            var sb = new StringBuilder();
            ...
            if (ordenar)
            {
                if (ordenarPorCampo)
                    ...
                    sb.AppendLine(" ORDER BY cat.Nome");
            }
